...
def _tabla_select_plana(datos):
    """.select(...).execute().data -- sin ningún .eq/.order (el select-todo de _mapa_hijos_por_puesto)."""
    tabla = MagicMock()
...
def _tabla_select_doble_eq(datos):
    """.select(...).eq(...).eq(...).execute().data -- dos cadenas .eq."""
    tabla = MagicMock()
...
def _fake_client_otorgados(filas_bitacora, filas_usuario=None):
    fake_client = MagicMock()
    tabla_mock = fake_client.postgrest.schema.return_value.table

    def side_effect(nombre_tabla):
        tabla = MagicMock()
        if nombre_tabla == "bitacora_movimiento_puesto_permiso":
            tabla.select.return_value.order.return_value.execute.return_value.data = filas_bitacora
        elif nombre_tabla == "usuario":
            tabla.select.return_value.in_.return_value.execute.return_value.data = filas_usuario or []
        return tabla
...
def test_listar_otorgados_resuelve_registrado_por_nombre():
    fake_client = _fake_client_otorgados(
        filas_bitacora=[
            {
        ...
        filas_usuario=[{"auth_user_id": CALLER_AUTH_USER_ID, "nombre_usuario": "caller.ficticio"}],
    )
    app.dependency_overrides[get_caller_client] = lambda: fake_client
...
def test_otorgar_puesto_inactivo_devuelve_422():
    fake_client = _fake_client_secuencia(
        [("puesto", _tabla_select_simple([{"id": PUESTO_DESTINO_ID, "activo": False}]))]
    )
...
def test_revocar_no_encontrado():
    fake_client = _fake_client_secuencia([("puesto_permiso", _tabla_select_simple([]))])
    _override_identidad(fake_client)
