-- 76_tiempo_dia_revision_admite_cerrado.sql
...
-- dia_update_revision (62_*.sql) como el chequeo interno de fn_dia_revisar (62_/65_/73_*.sql)
...
--   73_tiempo_fn_dia_revisar_resuelve_excepcion_de_dia.sql
...
-- sin cláusula explícita, desde 62_*.sql:103 hasta 73_*.sql:15) -- no hay nada que repetir acá.
-- ============================================================================
...
-- excepciones de marca Y de día (73_*.sql) -- queda idéntico: ya es agnóstico al estado de origen,
...
BEGIN
  ...
  -- éxito de arriba ya es el evento que las resuelve (SCJ-DEC-07, 73_*.sql). Un día que llegó
...
COMMENT ON FUNCTION tiempo.fn_dia_revisar(bigint, numeric) IS
  ...
  'una marca tardía después que fn_dia_calcular_armado_tramos sí puede emparejar -- 76_*.sql, '
  ...
  '76_tiempo_dia_revision_admite_cerrado.sql para el mapeo exacto que debe hacer backend. '
